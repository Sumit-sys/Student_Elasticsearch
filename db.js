...
connection.connect(err => {
  ...
  connection.query(createDatabaseAndTable, err => {
    ...
    // for (let i = 0; i < 500; i++) {
    // //   const full_name = faker.person.findName();
    // const full_name = faker.person.fullName(); // ✅ Correct

    // //   const age = faker.datatype.number({ min: 17, max: 25 });
    // faker.number.int({ min: 17, max: 25 }); 

    //   const gender = genders[Math.floor(Math.random() * genders.length)];
    //   const email = faker.internet.email();
    //   const address = faker.address.streetAddress() + ', ' + faker.address.city();
    //   const gpa = (Math.random() * (4.0 - 2.0) + 2.0).toFixed(2);
    //   const enrollment_year = faker.datatype.number({ min: 2018, max: 2025 });
